...
export function renderNav({ profile, active }) {
  const root = document.getElementById("nav-root");
  if (!root) return;

  const isProvider = profile.role === "provider";
  const home = isProvider ? "provider-dashboard.html" : "browse.html";
      ...
      <div class="max-w-6xl mx-auto px-5 sm:px-6 py-3 flex items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-3">
          <a href="edit-profile.html" class="flex items-center gap-2 group" title="Edit profile">
            <span class="hidden sm:block font-mono text-xs text-inksoft group-hover:text-ink transition">${escapeHtml(profile.name)}</span>
            ${avatarHtml(profile)}
          </a>
